...
const { Models } = require('@cyclonedx/cyclonedx-library')
const fs = require('fs')
const { LicenseFetcher } = require('../../dist/builders')
const { jest, describe, expect, it } = require('@jest/globals')

describe('LicenseFetcher', () => {
  const licenseFetcher = new LicenseFetcher()
  it('fetches existing license from directory', async () => {
    ...
    const licenses = licenseFetcher.fetchLicenseEvidence('test_module')
